...
from datasets.HelmNIO import HelmNIODataset
import cmocean

model = torch.load("Example/model.pkl", map_location=device)
...
with torch.no_grad():
    for i, (inp, target) in enumerate(eval_dataset):
        ...
        for j in range(n_plot):
            ...
            axes[j, 1].axis('off')
        plt.tight_layout()
        plt.savefig(f"src/visualization/images/eval_helmholtz/eval_batch_{i}.png")
        plt.close()
print("Errors:", errors)
