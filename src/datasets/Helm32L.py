...
class Helm32LDataset(Dataset):
    ...
    def __getitem__(self, index):
        inputs = torch.from_numpy(self.reader_test["training"]['sample_' + str(index + self.start)]["input"][:]).type(torch.float32)
        labels = torch.from_numpy(self.reader_test["training"]['sample_' + str(index + self.start)]["output"][:]).type(torch.float32)
