...
class CurveVelDataset(Dataset):
    def __init__(self, norm, inputs_bool, device, which, noise=0, mod="nio"):
        ...
        self.min_model = torch.tensor(1500.0)
        self.max_model = torch.tensor(4500.0)

        self.min_data = torch.tensor(-26.95)
        self.max_data = torch.tensor(52.77)

        self.min_data_logt = -3.3361253849184407
        self.max_data_logt = 4.027135812528651
