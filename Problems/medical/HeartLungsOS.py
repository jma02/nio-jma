...
class HeartLungsOSDataset(Dataset):
    ...
    def __getitem__(self, index):
        inputs = self.reader_testing[self.which]['sample_' + str(index + self.start)]["input"][:]
        inputs = np.delete(inputs, [16], axis=0)
        inputs = np.delete(inputs, [16], axis=1)
        inputs = torch.from_numpy(inputs).type(torch.float32)
        labels = torch.from_numpy(self.reader_testing[self.which]['sample_' + str(index + self.start)]["output"][:]).type(torch.float32)
